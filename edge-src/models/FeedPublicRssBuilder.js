...
export default class FeedPublicRssBuilder {
  ...
  _buildItemsRss() {
    ...
    this.jsonData.items.forEach((item) => {
      ...
      if (
        _microfeed["itunes:title"] &&
        _microfeed["itunes:title"].trim().length > 0
      ) {
      ...
      if (_microfeed["spotify:url"]) {
        itemJson["spotify:url"] = _microfeed["spotify:url"];
      }

      if (_microfeed["youtube:url"]) {
        itemJson["youtube:url"] = _microfeed["youtube:url"];
      }

      if (
        ["full", "trailer", "bonus"].includes(_microfeed["itunes:episodeType"])
      ) {
      ...
      if (mediaFile && mediaFile.url && mediaFile.url.length > 0) {
        ...
        if (
          mediaFile.duration_in_seconds &&
          mediaFile.duration_in_seconds > 0
        ) {
          itemJson["itunes:duration"] = secondsToHHMMSS(
            mediaFile.duration_in_seconds
          );
        }
      }
  ...
  _buildChannelRss() {
    ...
    if (this.jsonData._microfeed.items_next_cursor) {
      ...
      linksTags.push({
        "@_rel": "next",
        "@_href": `${PUBLIC_URLS.rssFeed(
          this.baseUrl
        )}?next_cursor=${items_next_cursor}&sort=${items_sort_order}`,
        "@_type": "application/rss+xml",
      });
    }
    if (this.jsonData._microfeed.items_prev_cursor) {
      ...
      linksTags.push({
        ...
        "@_href": `${PUBLIC_URLS.rssFeed(
          this.baseUrl
        )}?prev_cursor=${items_prev_cursor}&sort=${items_sort_order}`,
        "@_type": "application/rss+xml",
      });
    }
    ...
    if (
      this.jsonData.authors &&
      this.jsonData.authors.length > 0 &&
      this.jsonData.authors[0].name
    ) {
    ...
    if (
      _microfeed["itunes:email"] &&
      _microfeed["itunes:email"].trim().length > 0
    ) {
    ...
    if (
      _microfeed["itunes:new-feed-url"] &&
      _microfeed["itunes:new-feed-url"].trim().length > 0
    ) {
      channelRss["itunes:new-feed-url"] =
        _microfeed["itunes:new-feed-url"].trim();
    }
    if (_microfeed["itunes:block"]) {
